...
def HomePage(request):
    if request.method == 'POST':
        ...
        history = OrderHistory.objects.create(**history)
        send_order_update_message(history, user.id, history.id)
...
def send_order_update_message(order_history, user_id, order_id):
    ...
    message_data = {
        "userid": user_id,
        "orderid": order_id,
        "letters": {letter: getattr(order_history, letter, 0) for letter in letters}
    }
